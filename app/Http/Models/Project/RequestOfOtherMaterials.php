...
class RequestOfOtherMaterials extends Model
{
     /**
     ...
     * @var string
     */
    protected $table = 'request_of_other_materials';
    ...
    {
        return $this->hasMany('App\Http\Models\Project\RequestOfOtherMaterialItems', 'request_of_other_material_id');
    }
    ...
    public static function datatables($start, $length, $order, $dir, $search, $filter = '', $session = [])
    {
        ...
        $qry = self::select($_select)->addSelect('spk_workers.number as spk_number')
                    ->leftJoin('spk_workers', 'spk_workers.id', '=', 'request_of_other_materials.spk_id');
    ...
    public static function createOrUpdate($params, $method, $request)
    {
        ...
        $request_material['title'] = isset($params['title']) && $params['title'] ? $params['title'] : 'Request Pengajuan Bahan Lainnya';
        ...
        if (isset($params['id']) && $params['id']) {
            ...
            RequestOfOtherMaterialItems::whereRequestOfOtherMaterialId($id)->delete();

            foreach($params['item_name'] as $key => $val) {
                
                $request_material_item['request_of_other_material_id'] = $id;
                $request_material_item['inventory_name'] = $val;
                $request_material_item['brand'] = $params['item_brand'][$key];
                $request_material_item['unit'] = $params['item_unit'][$key];
                $request_material_item['qty'] = $params['item_qty'][$key];

                RequestOfOtherMaterialItems::create($request_material_item);
            }
        ...
        if ($insert) {
            if (isset($params['item_name']) && count($params['item_name']) > 0) {
                foreach($params['item_name'] as $key => $val) {
                    $request_material_item['request_of_other_material_id'] = $insert->id;
                    $request_material_item['inventory_name'] = $val;
                    $request_material_item['brand'] = $params['item_brand'][$key];
                    $request_material_item['unit'] = $params['item_unit'][$key];
                    $request_material_item['qty'] = $params['item_qty'][$key];
    ...
    public static function getPaginatedResult($params)
    {
        ...
        $db = self::select($_select)->addSelect('spk_workers.number as spk_number')
                    ->join('spk_workers', 'spk_workers.id', '=', 'request_of_other_materials.spk_id');
